  ...
  return (
    <article className="asset-card">
      <header>
        ...
        <button type="button" className="asset-title" onClick={onOpen}>{asset.filename}</button>
...
function Preview({ asset }: { asset: Asset }) {
  ...
  if (type.startsWith('image/') || type.startsWith('video/')) {
    return <AuthImage src={assetHubPath(`/v1/assets/${asset.id}/thumbnail?width=320&height=180&format=jpg`)} alt="" />
  }
